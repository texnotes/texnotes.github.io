...
(function ($) {
	...
	Slider.prototype.cycleSlider = function () {
		...
		if (currentSlide > SLIDES_NUM) {
			...
			$(SLIDE_FRAME, this.node).animate({"left" : "-=910px"}, 1000, "swing", function () {
			$(SLIDE_FRAME, this.node).css({"left" : 0});
			});
		} else {
			this.currentSlide = currentSlide;
			$(SLIDE_FRAME, this.node).animate({"left" : "-=910px"}, 1000, "swing");
		}
		var navPanel = $(NAVIGATION, this.node).children('.selected').removeClass('selected');
		var kids = $(NAVIGATION, this.node).children();
	...
	Slider.prototype.moveSlideRight = function (num) {
		num = num || 1;
		var data = 910 * num;
		$(SLIDE_FRAME, this.node).animate({"left" : "+=" + data + "px"}, "slow");
	}
	...
	Slider.prototype.init = function () {
		...
		var sliderImages = $(SLIDE_FRAME, this.node).find('img');//Для плавности показа добавляем псевдо-слайд дублирующий нулевой	
		if ((sliderImages[0].src) !== (sliderImages[(sliderImages.length - 1)].src)) { 
			var addPseudoImg = $(SLIDE_FRAME, this.node).children();
			$(SLIDE_FRAME, this.node).append("<li>" + addPseudoImg[0].innerHTML + "</li>");
		}
		this.controlsNav = this.node.find(NAVIGATION, this.node);
