...
//Комментарии к коду - только объяснение почему так делал для домашки, а не для финального кода!
//Блок проверок обязательно выполнить до серверной валитдации, которая будет работать асинхронно.		
//Поскольку используется серверная валитдация, то сверку с глобальным массивом можно было бы убрать. 
//Оставлена, чтобы хоть как-то реализовать логику проверок. 
...
//Валитдация почтового ящика на сервере. 
//Операция асинхронна и теоретически может не успеть завершиться до отправки формы.
//Последнее - уже задача сервера, который все-равно должен еще раз валитдировать почту при приеме формы. 
		
		$.getJSON( "/check-email/?email="+testEmail, function(json) {
				if (json.used === true) {
			
				globalEmailList.push(testEmail);//На текущий момент - это заплатка. 
				//Добавляет в глобальный массив существующий ящик.
				//Можно было бы еще сделать проверку на существование в глобальном массиве,
				//но в контексте всего кода это не добавит особых приемуществ.
...
//Прошлую домашку на валитдацию нужно было реализовать с использованием переменных-флагов для валитдации.
//Асинхронный код просто ломает логику, т.к. return false - приходит уже не от validateEmail().
 
//На будущее - лучше всегда предусмотреть расширения функционала, предполагая возможность вставки асинхронного кода.  
//Проблема понятна, с клиент-сервером разобрался. Поскольку еще не решено первое задание, предыдущий код не переделываю.
